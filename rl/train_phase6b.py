...
from train_phase6 import (TABLE_LENGTH, TABLE_WIDTH, R,
...
def first_ball_struck(cue_pos, aim_angle, balls_dict):
    ...
    for bid, (bx, by) in balls_dict.items():
        dx, dy = bx - cx, by - cy
        t = dx * vx + dy * vy
        if t <= 0:  # ball is behind the cue
            continue
        perp_sq = max(0.0, (dx * dx + dy * dy) - t * t)
        if perp_sq > two_r_sq:  # cue will pass beside this ball
            continue
...
# Pocket aim points (throat midpoints for corners, mouth center for sides).
# Sourced from table_geometry so the renderer, sim, and policy stay in sync.
from table_geometry import POCKETS as _POCKETS, pocket_captures as _pcap
...
def called_pocket_index(cue_pos, aim_angle, ball_pos, max_perp=3.0):
    ...
    A pocket only counts as the "called" pocket if the ball's exit ray passes
    within `max_perp` inches of the pocket center. This means the ball could
    actually enter the pocket on a straight-line shot (pocket radius = 2.5 corner,
    2.75 side). Default 3.0" allows a small margin.
    ...
    # Only accept if the best pocket is actually on the ball's path (within max_perp).
...
HEAD_SPOT = (25.0, 25.0)  # Standard head spot — fallback for break ball in rack area
HEAD_SPOT_ALT = (25.0, 20.0)  # Fallback-of-fallback if head spot is blocked by cue


def rerack_positions():
    """Return the 14 positions used on rerack (head-ball apex is empty)."""
    return RACK_POSITIONS[1:]  # 14 positions
...
def relocate_break_ball(remaining_pos, cue_pos):
    ...
    # Rack "area" = within radius 8" of rack apex. Any ball there interferes.
    if math.hypot(remaining_pos[0] - RACK_APEX[0],
        ...
        return list(HEAD_SPOT)  # last resort
